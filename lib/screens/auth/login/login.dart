              ...
              Center(
                child: Column(
                  children: [
                    ...
                    SizedBox(height: 16,),
                    Text("Welcome Back",style: TextStyle(fontSize: 24,fontFamily: 'PlusJakartaSans',fontWeight: FontWeight.w700),),
                    SizedBox(height: 4,),
                    Text("Log in now to continue exploring",style: TextStyle(fontSize: 16,fontFamily: 'PlusJakartaSans',color: graycol ),),
                    SizedBox(height: 90,),
                    Align(alignment: Alignment.centerLeft ,child: Text("Email Address",style: TextStyle(fontSize: 14,fontFamily: 'PlusJakartaSans',color: graycol,fontWeight: FontWeight.w500),)),
                    SizedBox(height: 8,),
                    TextField(
                    ...
                    }, child: Text("Forgot Password?",style: TextStyle(fontSize: 16,fontFamily: 'PlusJakartaSans',fontWeight: FontWeight.w600,color: black),),style: TextButton.styleFrom(minimumSize: Size(0, 0),padding: EdgeInsets.all(0)),)),
                    SizedBox(height: 40,),
                    SizedBox(height: 60,width: 390,
                        child: commonButton(text: "Log In" , txtSize: 18,context: context,
                          onPress: ()async  {
                          ...
                          final user = await LoginService().loginUser(email: loginController.emailController.text,
                              password: loginController.passwordController.text);
                          setState(() => isLoading =false);
                          if(user != null && user.status==1){
                            if(!mounted) return;
                            Navigator.push(context, MaterialPageRoute(builder: (context)=>FragmentScreen()));


                          }else{
                            if(!mounted) return;
                            ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(user?.msg ?? "Login Fail")));
                    ...
                    SizedBox(height: 40,),
                    Row(children:[ SizedBox(width: 70.5,),
                      Text("Are you new to Food Hygiene?",style: TextStyle(fontSize: 14,fontWeight: FontWeight.w500,color: graycol),),
                      TextButton(onPressed: (){
                        Navigator.push(context, MaterialPageRoute(builder: (context) => Signup()));
                      }, child: Text("Register",style: TextStyle(fontSize: 14,fontWeight: FontWeight.w500,color: black),),style: TextButton.styleFrom(padding: EdgeInsets.zero,minimumSize: Size(0, 0)),)
