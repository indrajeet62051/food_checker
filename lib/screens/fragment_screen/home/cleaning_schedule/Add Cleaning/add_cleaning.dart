...
import '../cleaning_schedule_main_screen.dart';
import 'add_cleaning_controller.dart';

class AddCleaning extends StatefulWidget {
  @override
  State<StatefulWidget> createState() => Addcleaning();
}

class Addcleaning extends State<AddCleaning> {
  late final AddCleaningController _controller;
  List<String> Location = ["Customer Area", "Kitchen", "HAll", "Back Office", "Stage"];
  late String SelectedLoc = Location[2];
  late String _selectedLocation = Location[0];

  List<String> Requirement =["Daily","Weekly","Monthly","Quarterly"];
  late String _selectedRequirement = Requirement[0];
  late String SelectedReq = Requirement[2];
  ...
  void initState() {
    super.initState();
    _controller = AddCleaningController();
    _controller.location.text = _selectedLocation;
  }
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  commonText(text: "Location", txtSize: 14),
                  Padding(
                    padding: const EdgeInsets.only(top: 8),
                    child: GestureDetector(
                      onTap: () async {
                        final newTemp = await _showLocationPicker(context);
                        if (newTemp != null) {
                          setState(() {
                            SelectedLoc = newTemp;
                          });
                  ...
                  commonText(text: "Requirement", txtSize: 14),
                  Padding(
                    padding: const EdgeInsets.only(top: 8),
                    child: GestureDetector(
                      onTap: () async {
                        final newTemp = await _showRequirementPicker(context);
                        if (newTemp != null) {
                          setState(() {
                            SelectedReq = newTemp;
                          });
  ...
  Future<String?> _showLocationPicker(BuildContext context) async {
    int selectedIndex = Location.indexOf(SelectedLoc);
                    ...
                    SizedBox(
                      ...
                      child: commonButton(
                        ...
                        onPress: () {
                          ...
                          Navigator.pop(context, Location[selectedIndex]);
                        },
                      ),
                    ),
                  ],
  ...
  Future<String?> _showRequirementPicker(BuildContext context) async {
    int selectedIndex = Requirement.indexOf(SelectedReq);
    String? selectedTemp = await showDialog<String>(
      context: context,
      builder: (BuildContext context) {
        return StatefulBuilder(
                  ...
                  children: [
                    ...
                    SizedBox(
                      ...
                      child: commonButton(
                        text: "Select",
                        onPress: () {
                          _controller.requirement.text =
                          Requirement[selectedIndex];
                          Navigator.pop(context, Requirement[selectedIndex]);
                        },
                      ),
                    ),
                  ],
